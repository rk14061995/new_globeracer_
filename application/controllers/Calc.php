...
class Calc extends CI_Controller {
    ...
    public function getStravaData_($user){
         ...
         foreach($regUser as $user){
            ...
            if($user->uId!=""){
                ...
                if($connectionType=="Strava"){
                    ...
                    // $this->getData($user);
                    $this->checkSunntoExpiryTime($user->user_email);
        ...
        $condition=array("race_registeration.to_show"=>1,"race_registeration.amoun_status"=>"Paid");
         ...
         foreach($regUser as $user){
            ...
            if($user->uId!=""){
                ...
                if($connectionType=="Strava"){
                    echo '<td> API ID: '.$user->uId.' : <span style="color:orange">'.$connectionType.'</span></td>';
                    $this->checkStravaExpiryTime($user->user_email);  
                }else{
                    echo '<td> API ID: '.$user->uId.' : <span style="color:blue">'.$connectionType.'</span></td>';
                    $this->checkSunntoExpiryTime($user->user_email);
                }
    ...
    public function checkSunntoExpiryTime($email){
        $user=$this->db->where('user_email',$email)->get('user_details')->row();
        // die();
    ...
    public function checkStravaExpiryTime($user_email){
        if($user_email!=""){
            echo '<td>'.$user_email.'</td>';
            $tokenDetails=$this->db->where('user_email',$user_email)->get('token_details')->row();
            if($tokenDetails){
                $refresh_token=$tokenDetails->refresh_token;
                $expires_at=$tokenDetails->expires_at;
                $expires_in=$tokenDetails->expires_in;
                $epoch = $expires_at;
                echo '<td>Updating Refresh Token .. For: '.$user_email.'</td>';
                echo '<td>Refresh Token: '.$refresh_token.'</td>';
                ...
                if($httpcode==200){
                    ...
                    $updateResp=$this->updateStravaAccessToken($result, $user_email);
    ...
    public function updateStravaAccessToken($data,$email){
        echo '<td> Updating .. </td>';
        $userData=array(
                        // "user_id"=>$id,
        ...
        if($this->db->where('user_email',$email)->update('token_details',$userData)){
    ...
    //To Get Athelete Data
    public function getUserDailyActivity(){
         $condition=array("race_registeration.to_show"=>1,"race_registeration.amoun_status"=>"Paid");
         $regUser=$this->db
         ->select('user_details.user_id as uId, user_details.user_email,user_details.id_table, user_details.lastname, user_details.firstname,user_details.connection_type, user_details.id_table as table_id, user_details.refresh_token, user_details.access_token ')
         ->where($condition)
         ...
         foreach($regUser as $user){
            ...
            if($user->uId!=""){
                ...
                if($connectionType=="Strava"){
                    echo '<td> API ID: '.$user->uId.' : <span style="color:orange">'.$connectionType.'</span></td>';
                    $this->getFromStrava($user->user_email);  
                }else{
                    echo '<td> API ID: '.$user->user_email.' : <span style="color:blue">'.$connectionType.'</span></td>';
                    $this->getFromSunnto($user->user_email);
                }
            }else{
                echo '<td style="color:red"> No API Connected</td>';
            }
    ...
    //To Fetch Data From Strava DB
    public function getFromStrava($email){
        $tokenDetails=$this->db->where('user_email',$email)->get('token_details')->row();
        $newDat=date('m/d/Y');
        ...
        if($httpcode==200){
            ...
            $insStatus=$this->insertEvent($result,$email);
            switch ($insStatus) {
                case 0:
                    echo '<td>Failed</td>';
                    return 0;
    ...
    public function insertEvent($data,$email){
        $eventDetailArray=json_decode($data);
        $temp=0;
        foreach($eventDetailArray as $eventDetail){
            //For Testing Purpose
            $createD=explode('T', $eventDetail->start_date_local);
            //Initially
            ...
            $eventData=array(
                            ...
                            // "avg_heart_rate"=>$eventDetail->average_heartrate,
                            // "max_heartrate"=>$eventDetail->max_heartrate
    ...
    //To Fetch Data From Sunnto DB
    public function getFromSunnto($email){
        $tokenDetails=$this->db->where('user_email',$email)->get('token_details')->row();
        $access_token=$tokenDetails->access_token;
      ...
      // echo ' Access Token: '.$access_token;
        ...
        if ($httpcode==200){
            ...
            switch ($type) {
                case 1:$t_ype="Running";break;
                case 2:$t_ype="Cycling";break;
                case 11:$t_ype="Hiking";break;
                case 0:$t_ype="Walking";break;
                case 70:$t_ype="Trekking";break;
                // case 1:$t_ype="Running";break;
                // case 1:$t_ype="Running";break;
                // case 1:$t_ype="Running";break;
                // case 1:$t_ype="Running";break;
